...
InitializedModelDetail = namedtuple("InitializeModelDetail", ["model_serial_number", "model", "param_grid_search", "model_name"])
...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6)->MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            logging.info(f"{'=='*20} Started evaluation model : [{type(model).__name__}]{'=='*20}")

            # getting prediction for training and testing dataset
            y_train_pred = model.predict(X_train)
            y_test_pred = model.predict(X_test)

            # calculating r2score on training and testing data
            ...
            # logging all the important metrics
            ...
            # If the model accuracy is greater than the base accuracy and train and test  score is whithin the threshold
        ...
        if metric_info_artifact is None:
            logging.info(f"No model found with higher accuracy than the base accuracy")
...
class ModelFactory:
    ...
    def class_for_name(module_name: str, class_name: str):
        try:
            # load the module, will raise ImportError if the module cannot be loaded
        ...
        try:
            ...
            for grid_searched_best_model in grid_searched_best_model_list:
                if base_accuracy < grid_searched_best_model.best_score:
                    ...
                    base_accuracy = grid_searched_best_model.best_model
